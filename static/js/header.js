(function initHeader() {
    ...
    header.innerHTML =
        ...
        '<div class="settings-bar">' +
            ...
            '<div class="settings-buttons auth-settings">' +
    ...
    function setAuthToken(token) {
        try {
            ...
            // Private browsing may reject storage. The request will fail closed.
        }
        updateAuthButton();
    }
    ...
    function createAuthDialog() {
        ...
        authDialog.innerHTML =
            '<form class="auth-dialog" role="dialog" aria-modal="true" aria-labelledby="auth-dialog-title">' +
                ...
                '<p id="auth-dialog-message">Worker APIを利用するにはAPIトークンが必要です。</p>' +
                ...
                '</div>' +
                '<small>トークンはこのタブのsessionStorageにのみ保存されます。</small>' +
            '</form>';
