...
- **処理**: ウィジェット内の商品ページ候補を収集し、Kindle / Audible / 電子書籍リンクを除外する。紙版候補を優先して商品詳細ページを取得する。
- 検索結果の商品詳細ページで ISBN-13 を取得できない場合でも、詳細ページの形式選択 (`tmm-grid-swatch-*`) でハードカバー等の紙版が選択されていれば、紙版の商品情報・表紙候補として扱う。
